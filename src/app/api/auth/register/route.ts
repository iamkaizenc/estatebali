...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only include verified if column exists and accepts it
    // If verified column doesn't have a default, we'll let the database handle it
    ...
    if (insertError) {
      ...
      if (insertError.message?.includes("column") || 
        ...
        if (!missingColumn) {
          ...
          } else if (insertError.message.toLowerCase().includes("user_type")) {
            missingColumn = "user_type";
        ...
        if (missingColumn) {
          let migrationFile = "supabase/migrations/fix_users_table_simple.sql";
          
          if (missingColumn === "id") {
            migrationFile = "supabase/migrations/ensure_users_id_column.sql";
          } else if (missingColumn === "user_type") {
            migrationFile = "supabase/migrations/fix_user_type_to_role.sql";
          }
          
          return NextResponse.json(
            { 
              success: false, 
              error: `Database schema needs to be updated. Please fix "${missingColumn}" column in users table. Run the migration: ${migrationFile}`,
              details: insertError.message,
              errorCode: insertError.code,
              hint: insertError.hint || `The users table needs the ${missingColumn} column to be fixed. If you see "user_type", it should be renamed to "role".`
            },
            { status: 500 }
          );
        } else {
          ...
          return NextResponse.json(
            { 
              success: false, 
              error: `Database schema error. Please run the migration: supabase/migrations/fix_user_type_to_role.sql to fix user_type/role columns, or supabase/migrations/fix_users_table_simple.sql for other missing columns.`,
              details: insertError.message,
              hint: "The users table needs password_hash, verified, and role columns. If you see 'user_type', it should be renamed to 'role'."
            },
            { status: 500 }
          );
        }
